  ...
  const groupList = useMemo(() => {
    ...
    return Array.from(groupIds)
      ...
      .map((groupId) => {
        ...
        const traits = summaryEntry?.traits ?? [];
        const traitsLine = traits.length ? traits.join(" · ") : "";
      ...
      <div className={expanded ? "mt-4 grid grid-cols-[minmax(0,1fr)_320px] gap-5 flex-1" : "mt-4 space-y-3"}>
        <div className={`relative ${mapHeight}`}>
          ...
          >
            ...
            {hoveredPoint && (() => {
              const hg = groupMap.get(hoveredPoint.groupId);
              const hColor = groupColor(hoveredPoint.groupId);
              return (
                <div
                ...
                >
                  <div className="w-[230px] rounded-xl border border-border bg-background/97 shadow-xl px-3.5 py-3 text-[11px]">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-1.5">
                        <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: hColor }} />
                        <span className="font-semibold text-foreground text-[12px] leading-tight">
                          {hg?.label || `Group ${hoveredPoint.groupId}`}
                        </span>
                      </div>
                      {hg && (
                    ...
                    {hg?.facts && (
                      <div className="mt-1.5 text-muted leading-snug text-[10px]">{hg.facts}</div>
                    )}
                    {hoveredPoint.summary && !hg?.facts && (
                      <div className="mt-1.5 text-foreground leading-snug">{hoveredPoint.summary}</div>
                    )}
                    {hg?.traits && hg.traits.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {hg.traits.slice(0, 5).map((trait, ti) => (
                          <span
                            key={ti}
                            className="px-1.5 py-0.5 rounded-full text-[9px] font-mono border border-border/70 text-muted"
                            style={{ backgroundColor: `${hColor}18` }}
                          >
                            {trait}
                          </span>
                        ))}
                    ...
                    {hg?.examples && (
                      <div className="mt-1.5 text-muted/70 leading-snug text-[10px]">
                        {hg.examplesLabel || "Example"}: {hg.examples}
                      </div>
                    )}
                  </div>
                </div>
...
export default function CampaignDetailPage() {
  ...
  useEffect(() => {
    ...
    document.title = name ? `${name} — ADsensei` : "Campaign — ADsensei";
  ...
  const GROUP_COLORS = ["#6366f1", "#f472b6", "#34d399", "#fbbf24", "#60a5fa", "#a78bfa"];
  const groupColor = (groupId: number) => GROUP_COLORS[groupId % GROUP_COLORS.length];
        ...
        <div className="absolute left-0 top-0 bottom-0 w-[400px] p-8 flex flex-col z-10">
          ...
          <div className="mt-6">
            {editingName ? (
              <input
                autoFocus
                className="text-2xl font-bold tracking-tight bg-transparent border-b border-foreground outline-none w-full"
              ...
              <h1
                ...
                onClick={() => { setCampaignName(campaign.name || ""); setEditingName(true); }}
                title="Click to edit"
              >
            ...
            {groupVariants.length > 0 && (
              <p className="mt-1 text-xs text-[#6366f1] font-medium">
                Generated {groupVariants.length} variants for {campaign.metadata?.groupCount || groupVariants.length} audience segments
              </p>
            )}
          ...
          <div className="mt-5">
            <a
              href="https://agenthansa.com"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#6366f1]/40 bg-[#6366f1]/8 text-[#6366f1] text-xs font-medium hover:bg-[#6366f1]/15 transition-colors"
            >
          ...
          <div className="mt-8 border-t border-border pt-6">
            ...
            <div className="mt-5">
              ...
              <button
                ...
                disabled={generating}
                className="cursor-pointer w-full px-5 py-2.5 bg-foreground text-background rounded-full text-xs font-medium hover:bg-foreground/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {generating ? (
                  ...
                  hasGeneratedAds ? "Regenerate variants" : "Generate variants"
                )}
              </button>
            </div>
          ...
          <div className="max-w-5xl">
            ...
            <div className="mt-12">
              <div className="flex items-center justify-between mb-4">
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted">Embeddings map</span>
                {embeddingsSource && !embeddingsLoading && (
                  <span className="font-mono text-[9px] uppercase tracking-widest text-muted">{embeddingsSource}</span>
                )}
              </div>
              {embeddingsLoading ? (
                <div className="border border-border rounded-2xl bg-foreground/2 p-4">
                  <div className="h-[280px] rounded-xl bg-foreground/4 animate-pulse" />
                  <div className="mt-3 grid grid-cols-3 gap-2">
                    {[0, 1, 2].map((i) => (
                      <div key={i} className="h-14 rounded-xl bg-foreground/4 animate-pulse" style={{ animationDelay: `${i * 120}ms` }} />
                    ))}
                  </div>
                </div>
              ) : embeddingsError ? (
                <div className="border border-dashed border-border rounded-2xl p-6 text-xs text-muted">
                  {embeddingsError}
                </div>
              ) : embeddings.length === 0 ? (
                <div className="border border-dashed border-border rounded-2xl p-6 text-xs text-muted">
            ...
            <div className="mt-12">
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted">
                  {groupVariants.length > 0 ? `${groupVariants.length} variants` : ""}
                </span>
              </div>
              {generating ? (
                <div className="mt-6 flex flex-col gap-6">
                  {Array.from({ length: effectiveGroupCount }).map((_, i) => (
                    <div key={i} className="rounded-2xl border border-border overflow-hidden" style={{ animationDelay: `${i * 150}ms` }}>
                      <div className="w-full aspect-video bg-foreground/4 animate-pulse" />
                      <div className="p-5 space-y-3">
                        <div className="flex items-center gap-2">
                          <div className="h-6 w-24 rounded-full bg-foreground/6 animate-pulse" />
                          <div className="h-6 w-16 rounded-full bg-foreground/4 animate-pulse" />
                          <div className="h-6 w-20 rounded-full bg-foreground/4 animate-pulse" />
                        </div>
                        <div className="h-3 w-3/4 rounded bg-foreground/4 animate-pulse" />
                        <div className="h-3 w-1/2 rounded bg-foreground/4 animate-pulse" />
                      </div>
                    </div>
                  ))}
                </div>
              ) : groupVariants.length === 0 ? (
                <div className="mt-6 border border-dashed border-border rounded-2xl p-10 flex flex-col items-center text-center gap-4">
                  <div className="w-14 h-14 rounded-full border border-dashed border-border flex items-center justify-center">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-muted/40">
                      <path d="M12 5v14M5 12h14" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-sm font-medium">No variants yet</p>
                    <p className="text-xs text-muted mt-1">Upload audience profiles and click Generate variants</p>
                  </div>
                </div>
              ) : (
                <div className="mt-6 flex flex-col gap-6">
                  {groupVariants.map((gv, index) => {
                    const color = GROUP_COLORS[gv.groupId % GROUP_COLORS.length];
                    const insights = gv.research?.insights?.slice(0, 120) + (gv.research?.insights && gv.research.insights.length > 120 ? "…" : "");
                    const appliedChanges = gv.changes?.filter((c) => c.applied) || [];
                    const colorGrade = appliedChanges.find((c) => c.tool === "color_grade_video")?.args?.style as string | undefined;
                    const speedChange = appliedChanges.find((c) => c.tool === "change_speed_video" || c.tool === "speed_up_video");
                    const overlayCount = appliedChanges.filter((c) => c.tool === "add_text_overlay_video").length;
                    const videoSrc = gv.variantUrl ? mediaUrl(gv.variantUrl) : null;

                    return (
                      <div key={gv.groupId} className="rounded-2xl border border-border overflow-hidden">
                        {/* Video */}
                        <div className="relative w-full aspect-video" style={{ backgroundColor: videoSrc ? "#0a0a0a" : color + "18" }}>
                          {videoSrc ? (
                            <video
                              ref={(el) => { if (el) variantRefs.current[`group-${gv.groupId}`] = el; }}
                              src={videoSrc}
                              controls
                              playsInline
                              ...
                              onError={(e) => {
                                ...
                                if (parent) {
                                  ...
                                  parent.setAttribute("data-broken", "1");
                                }
                              }}
                            />
                          ) : null}
                          {/* Broken / missing video fallback */}
                          {!videoSrc && (
                            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
                              ...
                              <div className="text-center">
                                <p className="text-sm font-semibold" style={{ color }}>{gv.label || `Group ${gv.groupId}`}</p>
                                <p className="text-xs text-white/40 mt-1">Variant rendering in progress</p>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="p-5">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              >
                                ...
                                {gv.label || `Group ${gv.groupId}`}
                              </span>
                              {colorGrade && (
                                <span className="px-2.5 py-1 rounded-full text-[11px] font-mono bg-foreground/6 text-muted border border-border">
                                  🎨 {colorGrade}
                                </span>
                              )}
                              {speedChange && (
                                <span className="px-2.5 py-1 rounded-full text-[11px] font-mono bg-foreground/6 text-muted border border-border">
                                  ⚡ {speedChange.args?.changeTag as string || "speed"}
                                </span>
                              )}
                              {overlayCount > 0 && (
                                <span className="px-2.5 py-1 rounded-full text-[11px] font-mono bg-foreground/6 text-muted border border-border">
                              ...
                              <span className="px-2.5 py-1 rounded-full text-[11px] font-mono bg-foreground/6 text-muted border border-border">
                                {appliedChanges.length} transforms
                              </span>
                            </div>
                            {videoSrc && (
                              <a
                                href={videoSrc}
                                download={gv.variantName || `variant-group-${gv.groupId}.mp4`}
                                className="shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-border text-[11px] text-muted hover:text-foreground hover:border-foreground/40 transition-colors"
                              >
                          ...
                          {gv.context?.interests && gv.context.interests.length > 0 && (
                            <div className="mt-3 flex flex-wrap gap-1">
                              {gv.context.interests.slice(0, 5).map((interest, ii) => (
                                <span key={ii} className="px-2 py-0.5 rounded-full text-[10px] bg-foreground/4 text-muted border border-border/50">
                                  {interest}
                                </span>
                              ))}
                            </div>
                          ...
                          <button
                            type="button"
                            onClick={() => setExpandedGroup(expandedGroup === gv.groupId ? null : gv.groupId)}
                            className="mt-3 font-mono text-[10px] uppercase tracking-widest text-muted hover:text-foreground transition-colors"
                          >
                            {expandedGroup === gv.groupId ? "Hide details ↑" : "Show details ↓"}
                          </button>
                        </div>
                      </div>
                    );
