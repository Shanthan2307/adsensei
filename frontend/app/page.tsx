...
export default function Home() {
  ...
  useEffect(() => {
    document.title = "ADsensei — AI Video Ad Localisation";
  }, []);
  ...
  const handleDemoLogin = async () => {
    ...
    setLoginError("");
    try {
      // Try register first (409 = already exists, that's fine)
      await api.auth.register("demo@test.com", "demo1234").catch((err) => {
        if (!err.message?.includes("409") && !err.message?.toLowerCase().includes("already")) throw err;
      });
      const response = await api.auth.login("demo@test.com", "demo1234");
            ...
            <div className="mt-8 text-center space-y-4">
              ...
              <div className="pt-4 border-t border-border">
                <button
                  ...
                  className="w-full px-8 py-3 bg-[#6366f1] text-white rounded-full text-sm font-medium hover:bg-[#4f46e5] transition-colors disabled:opacity-60 cursor-pointer"
                >
                  {demoLoading ? "Loading..." : "Try Demo →"}
                </button>
                <p className="mt-2 text-[11px] text-muted">No account needed</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col items-start gap-5 sm:gap-6 text-left max-w-4xl pointer-events-none">
            ...
            <div className="flex items-center gap-3" style={{ zIndex: 10000, position: "relative" }}>
              ...
              <button
                onClick={handleDemoLogin}
                disabled={demoLoading}
                className="cursor-pointer px-4 py-2 sm:px-6 sm:py-2.5 bg-[#6366f1] text-white rounded-full text-[12px] sm:text-sm font-medium hover:bg-[#4f46e5] transition-colors duration-200 pointer-events-auto disabled:opacity-60"
              >
                {demoLoading ? "Loading..." : "Try Demo →"}
              </button>
              <button
