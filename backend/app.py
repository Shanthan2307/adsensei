...
# Demo fast-path — pre-rendered variants for the demo campaign
DEMO_VIDEO_ID = "d20394fa1ce844a78268570a61881899"
_DEMO_SNAPSHOT_PATH = Path(__file__).parent / "demo_snapshot.json"
...
openai_client = OpenAI(api_key=os.getenv("OPENAI_API_KEY") or "", base_url=os.getenv("OPENAI_BASE_URL"))
...
async def generate_ads(video_id: str, payload: dict | None = None, user=Depends(get_current_user)):
    ...
    # ── Demo fast-path ──────────────────────────────────────────────────────
    # For the pre-rendered demo campaign, skip the real pipeline and return
    # the pre-built snapshot after a short delay so the UI animations play.
    if video_id == DEMO_VIDEO_ID and _DEMO_SNAPSHOT_PATH.exists():
        await asyncio.sleep(4)          # lets the frontend step animation run
        snapshot = json.loads(_DEMO_SNAPSHOT_PATH.read_text())
        group_variants = snapshot["groupVariants"]
        metadata = {
            "groupVariants": group_variants,
            "groupCount": snapshot["groupCount"],
            "generatedAt": datetime.now(timezone.utc).isoformat(),
        }
        # Persist so the campaign page shows variants on reload
        try:
            update_video_metadata(video_id, user["id"], metadata)
            delete_variants_by_prefix(video_id, "group-")
            for gv in group_variants:
                add_variant(video_id, gv["variantName"], gv["variantUrl"])
        except Exception:
            pass
        variants_out = [{"name": gv["variantName"], "url": gv["variantUrl"]} for gv in group_variants]
        return {"ok": True, "variants": variants_out, "metadata": metadata}
    # ── End demo fast-path ──────────────────────────────────────────────────
